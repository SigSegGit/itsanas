...
| **N** | P (dépôt), puis W et le Mac | Les noms de fichiers d'un vrai dossier. Depuis P : `Café décembre.txt` ; `Photo.JPG` **et** `photo.jpg` avec des contenus différents ; une arborescence d'environ 300 caractères ; `rapport final .txt` et `notes.` ; `CON.txt`. Regarde les trois machines | Même ensemble de fichiers partout, et **ce qui est refusé est refusé à voix haute**, en nommant le fichier. *Attendu en échec sur les trois premiers : rien ne normalise l'Unicode (le Mac décompose les accents → deuxième fichier), les paires qui ne diffèrent que par la casse sont acceptées ici et se télescopent sur W et le Mac, et `MAX_PATH_LEN` vaut 1024 contre 260 par défaut sur Windows. Voir `ROADMAP.md`, « Filenames across three operating systems ». N ne bloque pas le verdict A–M ; il bloque le fait de confier le dossier à quelqu'un qui a un Mac et des accents* |
