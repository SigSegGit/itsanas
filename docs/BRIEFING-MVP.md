...
| **N** | P (dépôt), puis W et le Mac | Les noms de fichiers d'un vrai dossier. Depuis P : `Café décembre.txt` ; `Photo.JPG` **et** `photo.jpg` avec des contenus différents ; une arborescence d'environ 300 caractères ; `rapport final .txt` et `notes.` ; `CON.txt`. Regarde les trois machines | **Les trois moitiés ont été testées le 2026-09-16 et aucune ne casse comme on le craignait.** Casse et chemins longs : OK sur W — `Camera/IMG.JPG` et `Camera/img.jpg` entrent en collision sur NTFS, le mécanisme de conflit **garde les deux** en nommant la copie, rien n'est perdu, et trois scans de plus ne bougent rien ; un chemin de 305 caractères s'écrit sans broncher. Accents : testé sur un vrai MacBook Air M4 — **APFS fusionne les deux écritures Unicode en un seul fichier** et conserve les octets du premier, donc un Mac ne redéposera pas les fichiers accentués reçus du Pi. Reste un cas de copie de conflit, déjà couvert. Voir `ROADMAP.md`, « Filenames across three operating systems »*
