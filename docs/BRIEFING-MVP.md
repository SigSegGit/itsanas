...
| **N** | P (dépôt), puis W et le Mac | Les noms de fichiers d'un vrai dossier. Depuis P : `Café décembre.txt` ; `Photo.JPG` **et** `photo.jpg` avec des contenus différents ; une arborescence d'environ 300 caractères ; `rapport final .txt` et `notes.` ; `CON.txt`. Regarde les trois machines | Même ensemble de fichiers partout, et **ce qui est refusé est refusé à voix haute**, en nommant le fichier. *Testé sur W le 2026-09-16 : la casse et les chemins longs **passent**. `Camera/IMG.JPG` et `Camera/img.jpg` arrivent sur NTFS, la seconde entre en collision, et le mécanisme de conflit **garde les deux** en nommant la copie (`img.local-<id>.jpg`) ; rien n'est perdu, et trois scans de plus ne bougent rien. Un chemin de 305 caractères s'écrit sans broncher. **Seul l'Unicode reste ouvert et non testé** : rien ne normalise les noms, le Mac décompose les accents, et il n'y a pas de Mac ici — attendu, un doublon plutôt qu'une perte. Voir `ROADMAP.md`, « Filenames across three operating systems »*
