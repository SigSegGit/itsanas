...
| M12 Android shell | `android/`, `crates/itsanas-android` | 🟨 **the app exists and runs** — 1354 lines of Kotlin (`MainActivity`, `SyncService`, `Vault`, `Account`) over 16 JNI calls that match `Native.kt` exactly, committed 2026-09-07 as "An Android application, and it runs". This row said "shell not written" until 2026-09-16, which was wrong and was repeated to Nicolas as fact. **What it cannot do** is join: `itsanas-android` has *no* coordinator support and *no* local discovery, and `login` takes the 24 words rather than a coordinator. A phone reaches the network only through `addPeer("ip:port")` typed by hand |
