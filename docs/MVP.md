...
| D — recovery from nothing | ✅ **by passphrase alone, on the fleet, 2026-09-21** | *2026-09-07, and this is the first time it was not four processes on one box.* The Freebox VM, which had never held the account `sigseg42`, was given its twenty-four words and nothing else: `itsanas login --username sigseg42 --phrase-file …` restored the identity with a **new device id for that machine**, `register` enrolled it with the coordinator on the Raspberry Pi, and one sync round pulled **four files** from the laptop over a real socket. `photo.bin` came back with the same SHA-256 as the original, byte for byte, and the canary in `note.txt` read correctly. The temporary device was then withdrawn with `itsanas device forget`, which is the other half of the same story: a machine used once must be able to stop counting. **The pass criterion was met on 2026-09-21: recovery by passphrase alone, through the escrow, across the internet.** A throwaway account `evasion` was created on the Freebox VM and lodged its sealed container with `register --recovery`. Two files were written (88 B and 400 000 B) and pushed to the **Raspberry Pi, which belongs to a different account** and holds them blind. The VM's node was then left switched off. On the laptop, a node with **no data and no recovery phrase** ran `itsanas login --username evasion --from ngas.fr:9898` with the passphrase alone: it restored the same account id (`942dd0e3…`), enrolled itself, and one `sync` against the Pi returned **both files, byte for byte** — `149ee0377dc7ca76…` and `b1e5c75c05411b5c…`, the digests taken on the VM before sending. Nothing was read from the machine that wrote them, because it was not running. The escrow, the coordinator and the host were all reached through `ngas.fr`, the public path. This row said ✅ until 2026-09-14 for the wrong reason; it is ✅ now for the right one |
...
| E — never awake together | ✅ **on the fleet, 2026-09-21** | `a_host_relays_one_device_to_another_that_it_never_met` in the laboratory, and through the kit between three local nodes on every push. **Then for real**: the same run as D above. The node that wrote the files was on the VM and was never started again; the node that read them was on the laptop, created afterwards. The two never spoke. Everything went through the Pi, which belongs to another account and cannot read a byte of what it relayed. What is still missing is a real power cycle rather than a process that was simply never started |
