...
Memory is an order of magnitude inside the 200 MiB the criterion asks for. **The number that was not being watched is the third column: three hundred megabytes written per day by a node with nothing to do** — a hundred gigabytes a year to store nothing new. Five hundred kilobytes per round against a handful of small transactions, so it is the storage engine's commit cost rather than the data, and which transaction dominates is not yet known. On an SSD it is unremarkable; on the SD card this project has already destroyed one of, it is not. Recorded rather than guessed at, and the next thing to measure |
