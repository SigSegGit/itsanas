...
| B — a file appears | 🟨 | **Across two operating systems**, verified: a 391 KiB file written on Windows, served on `0.0.0.0`, pulled by a Linux node of the same account over a real network interface, byte for byte. Two kernels, two filesystems, two C libraries, a network hop — which is what the Windows-laptop-to-Raspberry-Pi case is made of. Still one physical machine, and still nothing on ARM |
...
| D — recovery from nothing | ✅ *verified locally* | **The account and the files both come back.** Real binaries, four processes: a coordinator, a stranger's host, a laptop that writes five documents, edits one, deletes one, uploads and is then deleted from disk, and a replacement home that runs `itsanas login --username nicolas --from 127.0.0.1:19801` with the passphrase alone. Same user id, new device id, and after one `itsanas sync` against the host: every file back byte for byte, the **edit** rather than the original, the deleted file still deleted, `doctor` clean, and `status` reporting `placements 13 recorded` — so it knows where its data lives instead of believing itself the only copy. Not yet done across the real fleet |
