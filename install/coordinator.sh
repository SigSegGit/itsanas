...
# Pull the id out of the output rather than assuming the output *is* the id.
# On a fresh machine the binary generates the identity first and used to say so
# on stdout, which is the only run this script ever does: it died here with
# "could not read the coordinator's device id" and then printed the id in the
# same breath. The binary says that on stderr now, and this takes the last
# 64-hex-character token regardless, so a future extra line costs nothing.
RAW=$(su -s /bin/sh "$SERVICE_USER" -c \
    "/usr/local/bin/itsanas-coordinator --state '$STATE_DIR' --identity" 2>/dev/null)
DEVICE_ID=$(printf '%s\n' "$RAW" | tr -d '\r' | grep -oE '[0-9a-f]{64}' | tail -1)
case "$DEVICE_ID" in
    # Exactly sixty-four hex characters. The previous pattern was `[0-9a-f]*`,
    # which accepts "a" and accepts the empty string.
    ????????????????????????????????????????????????????????????????)
